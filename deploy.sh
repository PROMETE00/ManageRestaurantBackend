#!/bin/bash
# Script de verificación y despliegue - Backend
# Uso: ./deploy.sh

set -e

echo "🔙 Desplegando Backend de Restaurante..."
echo ""
...
while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
    if curl -s http://localhost:8081 > /dev/null; then
        echo "✅ Backend respondiendo en http://localhost:8081"
        break
    fi
...
echo "🌐 Acceso público: https://restaurantebackend.prome.works"
echo ""
echo "📋 Comandos útiles:"
echo "   Ver logs:        docker-compose logs -f"
echo "   Ver logs de DB:  docker-compose logs -f db"
